...
import { forgotUsername } from '../Utils/ApiUtils';
import FormError from './FormError';
import Loader from './Loader';

function ForgotUsername({ onClose, onSignIn }) {
  ...
  return (
      <PopupModal width={isForgotSuccess?'md:w-1/2 lg:w-1/4':''} title={isForgotSuccess?'Email sent':"Forgot your username"} onClose={onClose}>
       {!isForgotSuccess? <form className="w-full flex flex-col items-center justify-center gap-3">
          ...
          <button disabled={isLoading} onClick={handleSignUp} className="h-[60px] w-full flex justify-center items-center gap-2 rounded-[15px] bg-blue-600 text-xl text-white shadow">
             {!isLoading? <span>Submit</span> : <><span>Loading...</span> <Loader width={'w-4'} height={'h-4'} /></>}
      ...
      <div className=" w-3/4 items-center justify-start">
         <p className='font-medium text-green-700 text-center mb-4'>Forgot username email sent successfully!</p>
         <p className='text-center text-black'>Please check your email for more information regarding your username. </p>
      </div>
